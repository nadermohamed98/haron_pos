...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	$adjacents = 3;
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_order_supply where $quy left(date, 10) BETWEEN '$from' AND '$to'";
...
$sql = "SELECT *,left(date,10) as date FROM ".$prefix."_order_supply where $quy left(date,10) BETWEEN '$from' AND '$to' order by date DESC LIMIT $start, $limit";	
...
-->
 ...
 <?php $i++; } ?>   
    ...
    <th class="text-center"> <?php
         ...
         $totalSql="SELECT Sum(Quantity) as Quantity FROM ".$prefix."_order_supply where $quy left(date,10) BETWEEN '$from' AND '$to'";
...
echo  $Quantity;
    ...
    <th class="text-center"> <?php
        ...
        $totalSql="SELECT Sum(price) as Amount FROM ".$prefix."_order_supply where $quy left(date,10) BETWEEN '$from' AND '$to'";
        $result_get = mysqli_query($con,$totalSql);
        while($row_get = mysqli_fetch_array($result_get))
        {
...
$result_get = mysqli_query($con,"SELECT Total FROM ".$prefix."_order_supply where $quy left(date,10) BETWEEN '".$from."' AND '".$to."'");
