...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_sales_inv  where supplier='".$_GET['clientID']."' and left(date, 10) BETWEEN '$from' AND '$to'";
...
$sql = "SELECT * FROM ".$prefix."_sales_inv where supplier='".$_GET['clientID']."' and type='3' and left(date,10) BETWEEN '$from' AND '$to' order by $orderby $type LIMIT $start, $limit";	
...
$result_get = mysqli_query($con,"SELECT Total FROM ".$prefix."_sales_inv where supplier='".$_GET['clientID']."' and type='3' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
