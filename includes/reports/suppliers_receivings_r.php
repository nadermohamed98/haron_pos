...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_receivings_inv  where supplier=".$_GET['SupplierID']." and type='1' and  left(date, 10) BETWEEN '$from' AND '$to'";
 ...
 $sql = "SELECT * FROM ".$prefix."_receivings_inv where supplier=".$_GET['SupplierID']." and type='1' and left(date,10) BETWEEN '$from' AND '$to' order by $orderby $type LIMIT $start, $limit";
...
$result_get = mysqli_query($con,"SELECT Total,discount FROM ".$prefix."_receivings_inv where supplier=".$_GET['SupplierID']." and type='1' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
