...
while($row_search_suppliers = mysqli_fetch_array($result_search_suppliers))
  ...
  echo'<li class="'.$class.'"><a class="addSupplier" href="#" attr-name="'.$row_search_suppliers['name'].'" attr-id="'.$row_search_suppliers['id'].'">'.$row_search_suppliers['name'].'</a></li>';
  $i++;
   }
}else{
	echo'<li class="item2"><a >'.$no_data_lang.'</a></li>';
	}
  ?>
  </ul>
  </div>